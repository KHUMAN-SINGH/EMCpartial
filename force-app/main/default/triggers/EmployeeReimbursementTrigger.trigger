trigger EmployeeReimbursementTrigger on Employee_Reimbursement__c (after update, after insert,before insert) {
    ...
    if(Trigger.isInsert && Trigger.isBefore) {
        ...
        //Check Mileage Lock date Value is null or not time of insert new Record
        if(triggerCustomSetting.Mileage_Lock_date_on_Reimbursement__c == true) {
            EmployeeReimbursementTriggerHandler.checkLockDate(Trigger.New);
        }
    }
}